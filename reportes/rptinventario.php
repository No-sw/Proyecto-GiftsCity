...
$pdf->Cell(40,6,'Evento',1,0,'C',1);
$pdf->Cell(50,6,'Fecha inicial',1,0,'C',1);
$pdf->Cell(40,6,'Fecha final',1,0,'C',1);
...
if(isset($_GET["id"])){
    $idd=$_GET["id"];
    $rspta= $categoria->listar("SELECT * FROM `inventario` WHERE `idinventario`=".$idd." AND `condicion`=1");
}else{
    $rspta = $categoria-> listar("SELECT * FROM `inventario` WHERE `condicion`=1");
}




$pdf->SetWidths(array(40,50,40,60));
$ti=0;
$tit=0;
...
{  
    ...
    $tit+=$ti;
 	$pdf->SetFont('Arial','',7); 
    $pdf->Row(array(utf8_decode($nombre) ,$reg->fechaInicio,$reg->fechafinal,
    $reg->Descripcion));
}
$pdf->SetFont('Arial','B',10);
$pdf->Cell(20,6,'RTN:',1,0,'R',1);
$pdf->Cell(30,6,$emp->rtn,1,0,'R',1);
$pdf->Cell(30,6,'Telefonos:',1,0,'R',1);
$pdf->Cell(30,6,$emp->telefono,1,0,'R',1);
$pdf->Cell(30,6,'direccion:',1,0,'R',1);
$pdf->Cell(50,6,$emp->direccion,1,0,'R',1);
$pdf->Ln(6);
